...
export default function DancingPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const filtered = subjectsData.items.filter(
        s => s.category?.toLowerCase() === 'dancing'
      );
  ...
  const getDanceStyles = () => {
    return styles.filter(style => 
      style.styleName?.toLowerCase().includes('dance') || 
      style.styleName?.toLowerCase().includes('bharatanatyam') ||
      style.styleName?.toLowerCase().includes('hip-hop') ||
      style.styleName?.toLowerCase().includes('contemporary')
    );
  };
    ...
    <div className="min-h-screen bg-background text-foreground font-paragraph">
      ...
      <section className="relative w-full max-w-[100rem] mx-auto min-h-[60vh] flex items-center justify-center overflow-hidden mt-20">
        ...
        <div className="relative z-10 text-center px-8 md:px-16 lg:px-24 py-24">
          ...
          >
            Express yourself through movement and rhythm. Explore diverse dance styles from classical to contemporary.
          </motion.p>
          <motion.blockquote
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
      ...
      <section className="w-full max-w-[100rem] mx-auto px-8 md:px-16 lg:px-24 py-24" style={{ minHeight: '600px' }}>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {isLoading ? null : getDanceStyles().map((style, index) => (
            <motion.div
              key={style._id}
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              {style.styleImage && (
                <Image
                  ...
                  alt={style.styleName || 'Dance style'}
