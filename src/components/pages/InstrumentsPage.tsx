import { useEffect, useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { X, ArrowLeft, ExternalLink } from 'lucide-react';
import { BaseCrudService } from '@/integrations';
import { CreativeSubjects, SubjectStyles } from '@/entities';
import { Image } from '@/components/ui/image';
import Header from '@/components/Header';
import Footer from '@/components/Footer';

export default function InstrumentsPage() {
  const [instruments, setInstruments] = useState<CreativeSubjects[]>([]);
  const [styles, setStyles] = useState<SubjectStyles[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [selectedInstrument, setSelectedInstrument] = useState<CreativeSubjects | null>(null);
  const [selectedStyle, setSelectedStyle] = useState<SubjectStyles | null>(null);

  useEffect(() => {
    loadData();
  }, []);

  const loadData = async () => {
    try {
      const [subjectsData, stylesData] = await Promise.all([
        BaseCrudService.getAll<CreativeSubjects>('creativesubjects'),
        BaseCrudService.getAll<SubjectStyles>('subjectstyles')
      ]);

      const instrumentsFiltered = subjectsData.items.filter(
        s => s.category?.toLowerCase() === 'musical instruments'
      );
      setInstruments(instrumentsFiltered);
      setStyles(stylesData.items);
    } catch (error) {
      console.error('Error loading data:', error);
    } finally {
      setIsLoading(false);
    }
  };

  const getStylesForSubject = (subjectName: string) => {
    return styles.filter(style => 
      style.styleName?.toLowerCase().includes(subjectName.toLowerCase())
    );
  };

  const closeModal = () => {
    setSelectedInstrument(null);
    setSelectedStyle(null);
  };

  return (
    <div className="min-h-screen bg-background text-foreground font-paragraph">
      <Header />

      {/* Hero Section */}
      <section className="relative w-full max-w-[100rem] mx-auto min-h-[60vh] flex items-center justify-center overflow-hidden mt-20">
        <div className="absolute inset-0 bg-gradient-to-b from-primary/10 to-background" />
        <div className="relative z-10 text-center px-8 md:px-16 lg:px-24 py-24">
          <motion.h1
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
            className="font-heading text-6xl md:text-8xl text-foreground mb-8"
          >
            Find Your Sound
          </motion.h1>
          <motion.p
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.2 }}
            className="text-xl md:text-2xl text-foreground/80 max-w-3xl mx-auto mb-8"
          >
            Discover the joy and value of learning a musical instrument. From strings to percussion, find your perfect match.
          </motion.p>
          <motion.blockquote
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.4 }}
            className="text-lg md:text-xl text-primary italic max-w-2xl mx-auto"
          >
            "Music gives a soul to the universe, wings to the mind, flight to the imagination, and life to everything." — Plato
          </motion.blockquote>
        </div>
      </section>

      {/* Instruments Grid */}
      <section className="w-full max-w-[100rem] mx-auto px-8 md:px-16 lg:px-24 py-24" style={{ minHeight: '600px' }}>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {isLoading ? null : instruments.map((instrument, index) => (
            <motion.div
              key={instrument._id}
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.6, delay: index * 0.1 }}
              onClick={() => setSelectedInstrument(instrument)}
              className="group cursor-pointer relative overflow-hidden border border-primary/20 hover:border-primary transition-all duration-300 hover:shadow-[0_0_40px_rgba(218,165,32,0.2)]"
            >
              {instrument.heroImage && (
                <Image
                  src={instrument.heroImage}
                  alt={instrument.subjectName || 'Instrument'}
                  className="w-full h-80 object-cover group-hover:scale-110 transition-transform duration-500"
                  width={400}
                />
              )}
              <div className="absolute inset-0 bg-gradient-to-t from-background via-background/60 to-transparent opacity-80 group-hover:opacity-90 transition-opacity duration-300" />
              <div className="absolute bottom-0 left-0 right-0 p-6">
                <h3 className="font-heading text-2xl text-foreground group-hover:text-primary transition-colors duration-300">
                  {instrument.subjectName}
                </h3>
              </div>
            </motion.div>
          ))}
        </div>
      </section>

      {/* Modal Level 1 - Instrument Details */}
      <AnimatePresence>
        {selectedInstrument && !selectedStyle && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-background/95 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto"
            onClick={closeModal}
          >
            <motion.div
              initial={{ scale: 0.9, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.9, opacity: 0 }}
              transition={{ duration: 0.3 }}
              onClick={(e) => e.stopPropagation()}
              className="bg-background border-2 border-primary/30 max-w-5xl w-full max-h-[90vh] overflow-y-auto relative"
            >
              <button
                onClick={closeModal}
                className="absolute top-6 right-6 w-10 h-10 flex items-center justify-center bg-primary/10 hover:bg-primary text-foreground hover:text-primary-foreground transition-all duration-300 z-10"
                aria-label="Close"
              >
                <X className="w-6 h-6" />
              </button>

              <div className="grid md:grid-cols-2 gap-8 p-8 md:p-12">
                {/* Left - Image */}
                <div className="relative">
                  {selectedInstrument.heroImage && (
                    <Image
                      src={selectedInstrument.heroImage}
                      alt={selectedInstrument.subjectName || 'Instrument'}
                      className="w-full h-full object-cover"
                      width={600}
                    />
                  )}
                </div>

                {/* Right - Info */}
                <div className="flex flex-col">
                  <div className="flex-1">
                    <h2 className="font-heading text-4xl md:text-5xl text-foreground mb-6">
                      {selectedInstrument.subjectName}
                    </h2>
                    
                    {selectedInstrument.famousQuote && (
                      <blockquote className="text-lg text-primary italic mb-6 border-l-4 border-primary pl-4">
                        {selectedInstrument.famousQuote}
                      </blockquote>
                    )}

                    {selectedInstrument.description && (
                      <p className="text-foreground/80 mb-6 leading-relaxed">
                        {selectedInstrument.description}
                      </p>
                    )}

                    {selectedInstrument.learningTimeline && (
                      <div className="bg-primary/5 border border-primary/20 p-6 mb-8">
                        <h3 className="font-heading text-xl text-foreground mb-3">Learning Timeline</h3>
                        <p className="text-foreground/80">
                          {selectedInstrument.learningTimeline}
                        </p>
                      </div>
                    )}
                  </div>

                  {/* Choose Your Style */}
                  <div>
                    <h3 className="font-heading text-2xl text-foreground mb-6">Choose Your Style</h3>
                    <div className="grid grid-cols-2 gap-4">
                      {getStylesForSubject(selectedInstrument.subjectName || '').map((style) => (
                        <button
                          key={style._id}
                          onClick={() => setSelectedStyle(style)}
                          className="group relative overflow-hidden border border-primary/30 hover:border-primary p-6 text-left transition-all duration-300 hover:shadow-[0_0_20px_rgba(218,165,32,0.2)]"
                        >
                          <h4 className="font-heading text-lg text-foreground group-hover:text-primary transition-colors duration-300">
                            {style.styleName}
                          </h4>
                          {style.shortSummary && (
                            <p className="text-sm text-foreground/60 mt-2">
                              {style.shortSummary}
                            </p>
                          )}
                        </button>
                      ))}
                    </div>
                  </div>
                </div>
              </div>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>

      {/* Modal Level 2 - Style Details */}
      <AnimatePresence>
        {selectedStyle && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-background/95 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto"
            onClick={closeModal}
          >
            <motion.div
              initial={{ scale: 0.9, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.9, opacity: 0 }}
              transition={{ duration: 0.3 }}
              onClick={(e) => e.stopPropagation()}
              className="bg-background border-2 border-primary/30 max-w-5xl w-full max-h-[90vh] overflow-y-auto relative"
            >
              <button
                onClick={() => setSelectedStyle(null)}
                className="absolute top-6 left-6 w-10 h-10 flex items-center justify-center bg-primary/10 hover:bg-primary text-foreground hover:text-primary-foreground transition-all duration-300 z-10"
                aria-label="Back"
              >
                <ArrowLeft className="w-6 h-6" />
              </button>

              <button
                onClick={closeModal}
                className="absolute top-6 right-6 w-10 h-10 flex items-center justify-center bg-primary/10 hover:bg-primary text-foreground hover:text-primary-foreground transition-all duration-300 z-10"
                aria-label="Close"
              >
                <X className="w-6 h-6" />
              </button>

              <div className="grid md:grid-cols-2 gap-8 p-8 md:p-12">
                {/* Left - Image */}
                <div className="relative">
                  {selectedStyle.styleImage && (
                    <Image
                      src={selectedStyle.styleImage}
                      alt={selectedStyle.styleName || 'Style'}
                      className="w-full h-full object-cover"
                      width={600}
                    />
                  )}
                </div>

                {/* Right - Info */}
                <div className="flex flex-col justify-between">
                  <div>
                    <h2 className="font-heading text-4xl md:text-5xl text-foreground mb-6">
                      {selectedStyle.styleName}
                    </h2>

                    {selectedStyle.description && (
                      <p className="text-foreground/80 mb-8 leading-relaxed">
                        {selectedStyle.description}
                      </p>
                    )}
                  </div>

                  {/* Enroll Button */}
                  {selectedStyle.enrollmentFormUrl && (
                    <a
                      href={selectedStyle.enrollmentFormUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center justify-center gap-3 bg-primary text-primary-foreground px-8 py-5 text-lg font-heading hover:shadow-[0_0_30px_rgba(218,165,32,0.4)] transition-all duration-300"
                    >
                      Enroll for {selectedStyle.styleName}
                      <ExternalLink className="w-5 h-5" />
                    </a>
                  )}
                </div>
              </div>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>

      <Footer />
    </div>
  );
}
