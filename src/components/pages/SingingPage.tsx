...
export default function SingingPage() {
  ...
  const [selectedSubject, setSelectedSubject] = useState<CreativeSubjects | null>(null);
  ...
  const loadData = async () => {
    try {
      ...
      const filtered = subjectsData.items.filter(
        s => s.category?.toLowerCase() === 'singing'
      );
  ...
  const getStylesForSubject = (subjectName: string) => {
    return styles.filter(style => 
      style.styleName?.toLowerCase().includes('singing') || 
      style.styleName?.toLowerCase().includes('vocal')
    );
  };
    ...
    <div className="min-h-screen bg-background text-foreground font-paragraph">
      ...
      <section className="relative w-full max-w-[100rem] mx-auto min-h-[60vh] flex items-center justify-center overflow-hidden mt-20">
        ...
        <div className="relative z-10 text-center px-8 md:px-16 lg:px-24 py-24">
          ...
          >
            Express yourself through the power of your voice. From classical to contemporary, discover your unique sound.
          </motion.p>
          <motion.blockquote
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            "The voice is the only instrument made by God." — Unknown
          </motion.blockquote>
        </div>
      </section>
      ...
      <section className="w-full max-w-[100rem] mx-auto px-8 md:px-16 lg:px-24 py-24" style={{ minHeight: '600px' }}>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {isLoading ? null : getStylesForSubject('singing').map((style, index) => (
            <motion.div
              key={style._id}
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              {style.styleImage && (
                <Image
                  ...
                  alt={style.styleName || 'Singing style'}
